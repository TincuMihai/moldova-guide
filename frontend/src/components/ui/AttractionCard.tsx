...
export default function AttractionCard({ attraction }: { attraction: Attraction }) {
  ...
  return (
    <Link to={`/explore/${attraction.slug}`} className="group card-elevated overflow-hidden">
      ...
      <div className="p-4">
        <h3 className="font-display font-bold text-sm text-slate-800 dark:text-slate-200 mb-1 group-hover:text-brand-600 transition-colors line-clamp-1">{attraction.name}</h3>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-3 line-clamp-2">{attraction.shortDescription}</p>
