...
export default function LoginPage() {
  ...
  const handleSubmit = useCallback(async (ev: React.FormEvent) => {
    ...
    try {
      if (tab === 'login') await login({ email, password }); else await register({ name, email, password, role });
      // Role-based redirect
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card overflow-hidden shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            {tab === 'register' && <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Numele complet</label><input type="text" value={name} onChange={(e) => setName(e.target.value)} className={`input-field ${errors.name ? 'border-red-300' : ''}`} placeholder="Ion Popescu" />{errors.name && <p className="text-xs text-red-500 mt-1">{errors.name}</p>}</div>}
            <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Email</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className={`input-field ${errors.email ? 'border-red-300' : ''}`} placeholder="email@exemplu.md" />{errors.email && <p className="text-xs text-red-500 mt-1">{errors.email}</p>}</div>
            <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Parola</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className={`input-field ${errors.password ? 'border-red-300' : ''}`} placeholder="Minim 6 caractere" />{errors.password && <p className="text-xs text-red-500 mt-1">{errors.password}</p>}</div>
            {tab === 'register' && <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-2">Tip de cont</label><div className="grid grid-cols-2 gap-3">{roles.map((r) => (<button type="button" key={r.value} onClick={() => setRole(r.value)} className={`p-4 rounded-xl border-2 text-center transition-all ${role === r.value ? 'border-brand-500 bg-brand-50' : 'border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:hover:border-slate-700 dark:border-slate-700'}`}><span className="text-2xl block mb-1">{r.icon}</span><span className="text-xs font-semibold text-slate-700 dark:text-slate-300 block">{r.label}</span><span className="text-[10px] text-slate-400 block mt-0.5">{r.desc}</span></button>))}</div></div>}
          ...
          {tab === 'login' && <div className="px-6 pb-6"><div className="border-t border-slate-100 dark:border-slate-800 pt-4"><p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-3 text-center">Conturi demo:</p><div className="space-y-2">
            {[{ label: '🧳 Turist', email: 'tourist@demo.md', pass: 'tourist123' },{ label: '🗺️ Ghid', email: 'guide@demo.md', pass: 'guide123' },{ label: '🔑 Admin', email: 'admin@demo.md', pass: 'admin123' }].map((d) => (
              <button key={d.email} type="button" onClick={() => fillDemo(d.email, d.pass)} className="w-full flex items-center justify-between p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-sm"><span className="font-medium text-slate-600 dark:text-slate-400">{d.label}</span><span className="text-xs text-slate-400">{d.email}</span></button>
            ))}
          </div></div></div>}
        </div>
      </div>
