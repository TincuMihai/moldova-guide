...
import { tripService, attractionService } from '../../services';
import type { TripItem, TripDay, Attraction } from '../../types';
...
export default function TripPlannerPage() {
  ...
  useEffect(() => { attractionService.getAll().then(setAllAttractions); }, []);
  ...
  const addItem = useCallback((di: number, aid: string) => {
    const a = allAttractions.find((x) => x.id === aid); if (!a) return;
  ...
  const filtered = search.trim() ? allAttractions.filter((a) => a.name.toLowerCase().includes(search.toLowerCase())) : allAttractions;

  return (
    <div className="pb-16 page-shell">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="page-title mb-6">Planifica o calatorie</h1>
        <div className="card p-6 mb-6">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            <div className="sm:col-span-2"><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Titlu</label><input value={title} onChange={(e) => setTitle(e.target.value)} className={`input-field ${errors.title ? 'border-red-300' : ''}`} placeholder="Ex: Weekend la Chisinau" />{errors.title && <p className="text-xs text-red-500 mt-1">{errors.title}</p>}</div>
            <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Data inceput</label><input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="input-field" /></div>
            <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Data sfarsit</label><input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="input-field" /></div>
          </div>
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400 cursor-pointer"><input type="checkbox" checked={isPublic} onChange={(e) => setIsPublic(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-brand-500" />Calatorie publica</label>
        ...
        {days.length > 0 ? <div className="space-y-6">
          {days.map((day, di) => (
            <div key={day.date} className="card p-6">
              <div className="flex items-center justify-between mb-4"><h3 className="font-display font-bold text-slate-800 dark:text-slate-200">Ziua {di + 1} <span className="text-sm font-normal text-slate-400 ml-2">{day.date}</span></h3><button onClick={() => setAddDay(addDay === di ? null : di)} className="btn-secondary text-xs">+ Adauga loc</button></div>
              {addDay === di && <div className="mb-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700"><input value={search} onChange={(e) => setSearch(e.target.value)} className="input-field mb-3 text-sm" placeholder="Cauta atractii..." /><div className="max-h-48 overflow-y-auto space-y-1">{filtered.map((a) => (<button key={a.id} onClick={() => addItem(di, a.id)} className="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-white transition-colors text-left"><img src={a.images[0]} alt="" className="w-10 h-10 rounded-lg object-cover" /><div><p className="text-sm font-medium text-slate-700 dark:text-slate-300">{a.name}</p><p className="text-xs text-slate-400">{a.city}</p></div></button>))}</div></div>}
              {day.items.length > 0 ? <div className="space-y-2">{day.items.map((item, ii) => (
                <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 group">
                  ...
                  <img src={item.image} alt="" className="w-12 h-12 rounded-lg object-cover" /><div className="flex-1 min-w-0"><p className="text-sm font-medium text-slate-700 dark:text-slate-300 truncate">{item.name}</p><p className="text-xs text-slate-400">Durata: {item.duration}</p></div>
        ...
        </div> : <div className="card p-12 text-center"><p className="text-slate-500 dark:text-slate-400 mb-2">Completeaza datele de mai sus si apasa „Genereaza zilele"</p></div>}
      </div>
    </div>
  );
}