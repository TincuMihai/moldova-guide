  ...
  return (
    <div className="pb-16 page-shell">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="page-title mb-6">Locuri salvate</h1>
        <div className="flex flex-wrap gap-2 mb-6">{categories.map((c) => (<button key={c.value} onClick={() => setFilter(c.value)} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${filter === c.value ? 'bg-brand-500 text-white' : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}`}>{c.label} ({c.count})</button>))}</div>
          ...
          <div key={p.id} className="card overflow-hidden group">
            <div className="relative h-40 overflow-hidden"><img src={p.image} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              <div className="absolute top-3 right-3 flex gap-1">
                <button onClick={() => { setEditPlace(p); setEditNotes(p.notes); }} className="w-8 h-8 rounded-full bg-white/90 flex items-center justify-center hover:bg-white text-slate-600 dark:text-slate-400"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Z" /></svg></button>
                <button onClick={() => setDeleteId(p.id)} className="w-8 h-8 rounded-full bg-white/90 flex items-center justify-center hover:bg-red-50 text-slate-600 dark:text-slate-400 hover:text-red-500"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" /></svg></button>
              </div>
            </div>
            <div className="p-4"><h3 className="font-display font-bold text-sm text-slate-800 dark:text-slate-200 mb-1">{p.name}</h3><p className="text-xs text-slate-400 mb-2">{p.city}</p><StarRating rating={p.rating} size="sm" />{p.notes && <div className="mt-3 pt-3 border-t border-slate-50"><p className="text-xs text-slate-500 dark:text-slate-400 italic">{p.notes}</p></div>}</div>
          </div>
