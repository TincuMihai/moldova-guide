      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="card p-6 mb-6">
          <div className="flex items-start gap-4 mb-6">
            ...
            <div className="flex-1"><h1 className="font-display text-xl font-bold text-slate-900 dark:text-white">{user.name}</h1><p className="text-sm text-slate-500 dark:text-slate-400">{user.email}</p><span className="badge bg-brand-50 text-brand-600 border border-brand-200 mt-2">{ROLE_LABELS[user.role]}</span></div>
            <button onClick={() => setEditing(!editing)} className="btn-secondary text-sm">{editing ? 'Anuleaza' : 'Editeaza'}</button>
          </div>
          {editing ? (
            <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
              <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Nume</label><input value={name} onChange={(e) => setName(e.target.value)} className={`input-field ${errors.name ? 'border-red-300' : ''}`} />{errors.name && <p className="text-xs text-red-500 mt-1">{errors.name}</p>}</div>
              <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Telefon</label><input value={phone} onChange={(e) => setPhone(e.target.value)} className="input-field" placeholder="+373 ..." /></div>
              <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Despre mine</label><textarea value={bio} onChange={(e) => setBio(e.target.value)} className="input-field min-h-[80px] resize-none" /></div>
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3 text-sm text-slate-600 dark:text-slate-400">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">{stats.map((s) => (<div key={s.label} className="card p-4 text-center"><p className="font-display text-2xl font-bold text-slate-800 dark:text-slate-200">{s.value}</p><p className="text-xs text-slate-400 mt-0.5">{s.label}</p></div>))}</div>
        <div className="card p-6 mb-6"><h2 className="font-display font-bold text-base text-slate-900 dark:text-white mb-4">Insigne</h2><div className="grid grid-cols-3 sm:grid-cols-5 gap-4">{badges.map((b) => (<div key={b.name} className={`text-center p-3 rounded-xl ${b.earned ? 'bg-brand-50 border border-brand-100' : 'bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 opacity-40'}`}><span className="text-2xl block mb-1">{b.icon}</span><p className="text-[10px] font-semibold text-slate-700 dark:text-slate-300">{b.name}</p></div>))}</div></div>
