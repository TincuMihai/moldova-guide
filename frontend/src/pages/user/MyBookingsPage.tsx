  ...
  return (
    <div className="pb-16 page-shell">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="page-title mb-6">Rezervarile mele</h1>
        <div className="flex flex-wrap gap-2 mb-6">{tabs.map((t) => (
          <button key={t.value} onClick={() => setFilter(t.value)} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${filter === t.value ? 'bg-brand-500 text-white shadow-md' : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 hover:border-brand-200'}`}>{t.label}</button>
        ))}</div>
          ...
          <div key={b.id} className="card p-5">
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between gap-3"><div><h3 className="font-display font-bold text-base text-slate-800 dark:text-slate-200">{b.tourTitle}</h3><p className="page-subtitle">Ghid: {b.guideName}</p></div><StatusBadge status={b.status} /></div>
                <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-slate-400"><span>{b.date}</span><span>{b.participants} participanti</span><span className="font-semibold text-slate-600 dark:text-slate-400">{b.totalPrice} {b.currency}</span></div>
