    ...
    <div className="pb-16 page-shell">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <nav className="flex items-center gap-2 text-sm text-slate-400 mb-8">
          ...
          <span className="text-slate-600 dark:text-slate-400 font-medium truncate">{tour.title}</span>
        </nav>

        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div><h2 className="font-display text-xl font-bold text-slate-900 dark:text-white mb-3">Despre acest tur</h2><p className="text-slate-600 dark:text-slate-400 leading-relaxed">{tour.description}</p></div>
            {/* Itinerary */}
            <div>
              <h2 className="font-display text-xl font-bold text-slate-900 dark:text-white mb-4">Itinerariu</h2>
                  ...
                  <div key={stop.id} className="flex gap-4">
                    ...
                    <div className="flex-1 pb-6">
                      <div className="flex flex-col sm:flex-row gap-4">
                        <div className="flex-1"><h3 className="font-display font-semibold text-slate-800 dark:text-slate-200 mb-1">{stop.name}</h3><p className="text-sm text-slate-500 dark:text-slate-400 mb-1">{stop.description}</p><span className="text-xs text-slate-400">Durata: {stop.duration}</span></div>
            ...
            <div className="card p-6">
              <h2 className="font-display text-xl font-bold text-slate-900 dark:text-white mb-4">Ghidul tau</h2>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-display font-bold text-slate-800 dark:text-slate-200">{tour.guide.name}</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-2">{tour.guide.languages.join(' · ')}</p>
                  <StarRating rating={tour.guide.rating} reviewCount={tour.guide.reviewCount} />
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mt-2">{tour.guide.bio}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <Modal isOpen={showBook} onClose={() => { setShowBook(false); setBSuccess(false); }} title={bSuccess ? 'Rezervare confirmata!' : 'Rezerva turul'}>
        {bSuccess ? (
          <div className="text-center py-4">
            ...
            <p className="text-slate-600 dark:text-slate-400 mb-4">Rezervarea ta a fost trimisa cu succes!</p>
